// secure storage
const String userKey = 'user';

// constants for api

const String baseUrl = 'meals.dotprogrammers.com';
const String register = 'api/register';
const String login = 'api/login';
const String forgotPassword = 'api/forgot-password';
const String resetPassword = 'api/password-reset-store';
const String sliderUrl = 'api/user/sliders';
const String itemsUrl = 'api/user/items';
const String itemDetailUrl = 'api/user/item-details';
const String itemByCategoryUrl = 'api/user/items-by-category';
const String packagesUrl = 'api/user/packages';
const String packageDetailUrl = 'api/user/package-details';
const String packageByCategoryUrl = 'api/user/packages-by-category';
const String cartItemsUrl = '/api/carts';
const String addToCartUrl = '/api/add-to-cart';
const String removeFromCartUrl = '/api/remove-cart';
const String favouriteItemsUrl = '/api/favourites';
const String addToFavouriteUrl = '/api/add-to-favourite';
const String removeFromFavouriteUrl = '/api/remove-favourite';
const String todaysMealUrl = '/api/user/todays-meals';
const String mealOnOffSettingsUrl = '/api/user/meal-settings';
const String mealOnOffUrl = '/api/meal-off-on';
const String guestMealUrl = 'api/guest-meal-on';
const String postUserLocationUrl = '/api/user-location-store';
const String getUserLocationUrl = '/api/user-location';
const String announcementUrl = '/api/user/announcements';
const String profileUrl = '/api/user-profile';
const String balanceUrl = '/api/balance';
const String changePasswordUrl = '/api/change-password';
const String orderUrl = '/api/order-now';
const String rechargeUrl = '/api/user-recharge-now';
