import 'dart:ui';

//assets
const String avatarDefault =
    'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTxdTDMVXzm38EXidxkpIan24gUYI0onfqcLQ&usqp=CAU';
const String alarm = 'assets/alarm.svg';
const String chat = 'assets/Chat.png';
const String create = 'assets/Create.png';
const String discount = 'assets/Discount.png';
const String avatar = 'assets/avatar.png';
const String foodDelivery = 'assets/Food delivery.png';
const String friedRice = 'assets/Fried rice.png';
const String hero = 'assets/hero.png';
const String location = 'assets/Location.png';
const String lunchTime = 'assets/Lunch time.png';
const String menu = 'assets/Menu.png';
const String dish = 'assets/Dish.png';
const String order = 'assets/order.svg';
const String paperBag = 'assets/Paper bag.png';
const String search = 'assets/Search.png';
// navbar
const String announcement = 'assets/Announcement.png';
const String cart = 'assets/Cart.png';
const String favourite = 'assets/Favourite.png';
const String home = 'assets/Home.png';
const String profile = 'assets/Profile.png';
// features icons
const String category = 'assets/Category.png';
const String createPackage = 'assets/Create Package.png';
const String deliveryTime = 'assets/Delivery Time.png';
const String feedback = 'assets/Feedback.png';
const String guestMeal = 'assets/Guest Meal.png';
const String help = 'assets/Help.png';
const String mealOnOff = 'assets/Meal Off On.png';
const String myOffer = 'assets/My Offer.png';
const String myReport = 'assets/My Report.png';
const String officialPackage = 'assets/Official Package.png';
const String todaysMenu = 'assets/Todays Menu.png';

// category screen
const String breakfast = 'assets/Breakfast.png';
const String crisp = 'assets/Crisps.png';
const String cupcake = 'assets/Cupcake.png';
const String saleTag = 'assets/Sale tag.png';
const String turkey = 'assets/Turkey.png';

// welcome screen
const String logo = 'assets/logo.png';
const String welcome = 'assets/welcome.png';
// login regiser screen
const String login = 'assets/login_image.png';
const String google = 'assets/google.png';
const String guest = 'assets/guest_star.png';

// verification screen
const String letter = 'assets/letter.png';
// colors
const Color primaryColor = Color(0xFFED1E24);
const Color secondaryColor = Color(0xFFE7E7E7);
const Color tertiaryColor = Color(0xFFF2F2F2);
const Color textColor = Color(0xFF000000);
const Color textSecondaryColor = Color(0xFF1E1E1E);
const Color neutralColor = Color(0xFFFFFFFF);

// text field
const Color textFieldColor = Color(0xFFF3F3F3);
const Color textFieldBorder = Color(0xFFB2AEAE);
const Color textFieldHint = Color(0xFF8E8E8E);
